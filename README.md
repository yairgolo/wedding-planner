...
### עדכון פורטל ההזמנות — החלטת הזמנה ומחיקה

לאחר `git pull` יש להפעיל את הסביבה הווירטואלית ולהריץ
`python -m flask --app run.py init-db` לפני Reload ב־PythonAnywhere.
הפקודה מוסיפה את העמודות החדשות ללא מחיקת הנתונים; מוזמנים קיימים נשארים מוזמנים.

אפשר לסמן מוזמן ״בסימן שאלה״ דרך הרשימה, העריכה או עמודת ״החלטת הזמנה״ ב־Excel.
השליחה חסומה עד לשינוי ל״כן, מזמינים״. סטטוס שליחה קודם נשמר כהיסטוריה.
מחיקה זמינה תחת ״סטטוס ופרטים נוספים״ ודורשת אישור; שחזור זמין ב״מוזמנים שנמחקו״.
מוזמנים שנמחקו אינם נכללים בייצוא ואינם זמינים לשליחה. משתמש רואה ומשנה רק את הצד שלו.
